export class StringUtils {
  ...
  static camelCase(str) {
    return str.replace(/(?:^\w|[A-Z]|\b\w|\s+|\_+|\-+|\b)/g, (match, index) => index === 0 ? match.toLowerCase() : match.toUpperCase()).replace(/\s+/g, '').replace(/[\-_]+/g, '');
  }
  ...
  static slugify(str) {
    return str.toLowerCase().replace(/[^a-z0-9 -]/g, '') // Remove invalid characters
